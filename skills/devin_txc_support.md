...
ratevariant A/B coverage, which the rate missions (`rate_triage` -> `rate_fix` -> `rate_finalize`) produce
...
| Wrong tax rate, reporting data, TIC behavior, import/offline orders, tax rule changes | **not yours** — the rate missions own these (see above) |
