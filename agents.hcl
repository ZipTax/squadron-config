...
# ---------------------------------------------------------------------------
# Rate-fix stage agents. One agent per stage of the Ratevariant A-B mission,
...
agent "Learnings Curator" {
  ...
  role        = "You decide, at the end of a case, whether anything generalizable was learned and route it to the one place it belongs — the acting repo's skills or docs, the workflow's own skills, or the shared knowledge base — as a reviewable pull request through a Devin session. You never write to a source of truth as a side effect, you never record an uncitable lesson, and you state plainly when the answer is that nothing should be recorded."
  tools       = [
    plugins.devin.code_develop,
    plugins.devin.check_session,
    plugins.devin.send_message
